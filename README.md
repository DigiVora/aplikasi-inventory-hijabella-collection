# 📦 Aplikasi Inventory Hijabella Collection

Aplikasi Inventory Hijabella Collection adalah sistem informasi persediaan barang berbasis Java NetBeans dan MySQL yang digunakan untuk mengelola data barang, barang masuk, barang keluar, serta laporan persediaan secara efektif dan efisien.

<div align="center">

![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![Status](https://img.shields.io/badge/Status-Stable-brightgreen?style=for-the-badge)
![Studio](https://img.shields.io/badge/DigiVora--Studio-orange?style=for-the-badge)

</div>

---

## ✨ Fitur Utama

- Login dan Logout
- Manajemen Data Barang
- Manajemen Barang Masuk
- Manajemen Barang Keluar
- Pencarian Data
- Laporan Persediaan Barang
- About Application

---

## 🛠️ Teknologi yang Digunakan

| Teknologi | Keterangan |
|-----------|------------|
| Java | Bahasa Pemrograman |
| Java Swing | User Interface |
| Apache NetBeans IDE 29 | IDE Pengembangan |
| MySQL | Database |
| JDBC | Koneksi Database |

---

## 📂 Struktur Modul

- Login
- Dashboard
- Data Barang
- Barang Masuk
- Barang Keluar
- Laporan
- About

---

## 👥 Tim Pengembang

| No | Nama |
|----|------|
| 1 | Achmad Khusnul Yakin |
| 2 | Achmad Fathoni Shidqon |
| 3 | Aulia Asmarani |
| 4 | Eka Futkhiyana Sari |
| 5 | M. Tijani Wafa Haqiqi |
| 6 | Muhammad Atho'il Hakim |
| 7 | Raihan Chayyin |

---

## 🎯 Tujuan Pengembangan

Aplikasi ini dibuat untuk memenuhi tugas mata kuliah Pemrograman Lanjutan serta membantu proses pengelolaan persediaan barang pada UMKM Hijabella Collection.

---

## 🚀 Cara Menjalankan

1. Clone repository

```bash
git clone https://github.com/DigiVora/aplikasi-inventory-hijabella-collection.git
```

2. Import project ke Apache NetBeans.

3. Buat database MySQL.

4. Import file database yang tersedia.

5. Sesuaikan konfigurasi koneksi database.

6. Jalankan project.

---

## 📜 License

Project ini menggunakan lisensi MIT.

---

## 🤝 Hubungi Saya

<p align="center">
  <a href="https://www.linkedin.com/in/achmad-khusnul-yakin-25b87a3a2?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=android_app" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" style="margin-bottom: 5px;">
  </a>
  <a href="https://www.instagram.com/digivora.studio" target="_blank">
    <img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white" alt="Instagram" style="margin-bottom: 5px;">
  </a>
  <a href="https://wa.me/6283116152891" target="_blank">
    <img src="https://img.shields.io/badge/WhatsApp-25D366?style=for-the-badge&logo=whatsapp&logoColor=white" alt="WhatsApp" style="margin-bottom: 5px;">
  </a>
  <a href="https://github.com/DigiVora" target="_blank">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" style="margin-bottom: 5px;">
  </a>
</p>
<br>

---

<p align="center">
  <img src="https://img.shields.io/badge/Copyright%20%C2%A9%202026-DigiVora%20Studio-orange?style=flat-square" alt="Copyright">
  <br>
  <sub>Sistem ini dikembangkan dengan dedikasi oleh:
  <br>
  <b>Achmad Khusnul Yakin, S.Kom.</b></sub>
</p>
